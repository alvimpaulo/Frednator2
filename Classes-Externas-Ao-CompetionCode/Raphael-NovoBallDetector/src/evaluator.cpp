...
int dbg_circle(cv::Mat frame, cv::Point center, int radius, int color){
	#ifdef DEBUG
        if(color == 0) cv::circle( frame, center, radius, cv::Scalar(0,0,255), 3, 8, 0 );
        else if(color == 1) cv::circle( frame, center, radius, cv::Scalar(0,255,0), 3, 8, 0 );
        else if(color == 2) cv::circle( frame, center, radius, cv::Scalar(0,255,255), 3, 8, 0 );
	#endif
	return 0;
}
