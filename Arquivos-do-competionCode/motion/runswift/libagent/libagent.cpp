...
void Agent::doBattery(float charge, float current, int status) {
   ...
   if (!old_battery_status[5] && b[5]) {
      ////qiLogInfo("LibAgent") << "Discharging" << std::endl;
   } else if (!old_battery_status[6] && b[6]) {
      ////qiLogInfo("LibAgent") << "Fully Charged" << std::endl;
   } else if (!old_battery_status[7] && b[7]) {
      ////qiLogInfo("LibAgent") << "Charging" << std::endl;
   }
...
void Agent::doButtons(bool chest, bool left, bool right) {
   ...
   if (chest_up > MAX_CLICK_INTERVAL && chest_presses) {
      ...
      if (buttons.pop(2)) {
         ...
         if (left || right) {
            SAY("Reset now key");  // yay transliteration
            ...
            ret = system("/usr/bin/killall unbeatables");
            ret = system("sudo /sbin/init a");
            // should not get here
         } else {
            if (skipped_frames > MAX_SKIPS) {
               SAY("loaded unbeatables");
               if (!fork()) {
                  struct sched_param s;
                  s.sched_priority = 0;
                  int ret = sched_setscheduler(0, SCHED_OTHER, &s);
                //   ret = system("/usr/bin/killall runswift");
                  ret = system("/usr/bin/killall unbetables");
                //   ret = execlp("/home/nao/bin/runswift", "runswift", (char*)NULL);
                  ret = execlp("/home/nao/naoqi/unbeatables", (char*)NULL);
               }
            } else {
               SAY("killed unbeatables");
               ret = system("/usr/bin/killall unbetables");
               ret = execlp("/home/nao/naoqi/matador", (char*)NULL);
               sleep(3);
               ret = system("sudo /sbin/init a");
            }
         }
   ...
   if (chest_down > 300) {  // 3 seconds
      if (left || right) {
         ...
         SAY("Shutting down");
         ret = system("sudo '/usr/bin/killall unbeatables'");
...
void Agent::doHeadTouch(float headTouchFront,
   ...
   //This function is only called iff at least one of the three values are correct
...
void Agent::preCallback() {
   ...
   ////qiLogInfo("LibAgent") << "Limp:" << limp << " Data:" << shared_data->standing << " SkipFrames: "<< skipped_frames << " Sit Step:" << sit_step << std::endl;
   if (limp || shared_data->standing) {
      ...
      if (sit_step < 1.0f) {
         // interpolate legs over 2s, arms over 1s
         sit_step += 0.005f;
         uint8_t i;
         for (i = Joints::HeadYaw; i <= Joints::HeadPitch; ++i) {
   ...
   if(t.elapsed_us() > 1500){
      qiLogWarning("LibAgent") << "preCallback took: " << boost::lexical_cast<string>(t.elapsed_us()) << std::endl;
   }
}
...
void Agent::postCallback() {
   ...
   if (v[Sensors::Head_Touch_Front] != 0
      ...
      doHeadTouch(v[Sensors::Head_Touch_Front],
   ...
   if (i <= 0) {
      sem_post(semaphore);
      if (skipped_frames > MAX_SKIPS) {
         ////qiLogInfo("LibAgent") << "Back after " << boost::lexical_cast<string>(skipped_frames) << std::endl;
      }
      skipped_frames = 0;
   } else {
      skipped_frames++;
   ...
   if(t.elapsed_us() > 1500){
       qiLogWarning("LibAgent") << "postCallback took: " << boost::lexical_cast<string>(t.elapsed_us()) << std::endl;
   }
}
...
Agent::Agent(boost::shared_ptr<AL::ALBroker> pBroker, const std::string& pName)
   ...
   memory->insertData("GameCtrl/teamNumber", 31);
   memory->insertData("GameCtrl/teamColour", 0);
   memory->insertData("GameCtrl/playerNumber", 1);*/
   ...
   //sonar_pointers.push_back((float*) memory->getDataPtr("Device/SubDeviceList/US/Sensor/Value"));
   }*/
   ...
   if (!fork()) {
      ...
      s.sched_priority = 0;
      int ret = sched_setscheduler(0, SCHED_OTHER, &s);
      ret = execlp("/home/nao/bin/unbeatables",
            (char*)NULL);
      ::exit(ret);
   }
   */
...
Agent::~Agent() {
   ...
   {
       qiLogError("LibAgent") << e.what() << std::endl;
   }
   ...
   delete memory;
   delete speech;
   ////qiLogInfo("LibAgent") << "Destroyed" << std::endl;
   //delete log;
}
