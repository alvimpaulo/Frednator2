...
AgentTouch::AgentTouch() {
   ...
   if (pthread_mutex_trylock(&shared_data->lock)) {
      qiLogVerbose("Motion") << "Another runswift is already running!" << std::endl;
      std::cerr << "Another runswift is already running!" << std::endl;
      exit(1);
   }

   ////qiLogInfo("Motion") << "AgentTouch constructed" << std::endl;
}
...
AgentTouch::~AgentTouch() {
   ...
   ////qiLogInfo("Motion") << "AgentTouch destroyed" << std::endl;
}

SensorValues AgentTouch::getSensors(Kinematics &kinematics) {
    ////qiLogInfo("Motion") << "Get Sensors" << std::endl;
   sem_wait(semaphore);
