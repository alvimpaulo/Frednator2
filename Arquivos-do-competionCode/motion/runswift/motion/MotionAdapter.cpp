 ...
 *---------------------------------------------------------------------------*/
MotionAdapter::MotionAdapter(const boost::program_options::variables_map& config, UnBoard *ub)
   : uptime(0), prevActionID(-1) {

   this->unBoard = static_cast<UnBoard*>(ub);
...
//#endif
   ...
   generator = (Generator*) new ClippedGenerator(
      (Generator*) new DistributedGenerator(this->unBoard));
   if (generator == NULL) {
...
void MotionAdapter::tick() {
   ...
   ActionCommand::All request = readFrom(behavior, request[index].actions);
   float ballX = request.body.ballX;
   float ballY = request.body.ballY;

   bool isSameAction = (prevActionID == request.actionIndex);
   ...
   ButtonPresses buttonData;
   buttonData = touch->getButtons(); //Figure this out
   ...
   writeTo(motion, isStanding, standing);
   ...
   buttonData |= readFrom(motion, button);
   writeTo(motion, button, buttonData);
   ...
   AbsCoord ball(ballX, ballY, 0.0);

   // Update the body model
   bodyModel.kinematics = &kinematics;
   bodyModel.update(&odometry, sensors);

   Odometry diffOdometry = odometry - prevOdometry;

   if(abs(diffOdometry.forward) >= abs(request.body.forward) && abs(diffOdometry.left) >= abs(request.body.left) && abs(diffOdometry.turn) >= abs(request.body.turn))
   {
       //////qiLogInfo("TesteMotion") << "OdoF: " << diffOdometry.forward << "OdoT: " << diffOdometry.turn << " Id: " << request.actionIndex << " type: " << request.body.actionType << std::endl;
       if(isSameAction && request.body.actionType == ActionCommand::Body::WALK)
       {
           request.body.forward = request.body.left = request.body.turn = 0.0;
           writeTo(motion, walkFinished, true);
       }
   }
   if(!isSameAction)
   {
   ...
   prevActionID = request.actionIndex;
