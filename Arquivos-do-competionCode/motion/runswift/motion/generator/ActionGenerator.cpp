...
ActionGenerator::~ActionGenerator() {
   ////qiLogInfo("Motion") << "ActionGenerator destroyed" << std::endl;
};
...
JointValues ActionGenerator::makeJoints(ActionCommand::All* request,
   ...
   ////qiLogInfo("Motion") << "Make Action " << request->body.actionType << std::endl;
...
void ActionGenerator::interpolate(JointValues newJoint, int duration) {
   if (joints.empty()) {
      max_iter = duration / TEN_MS;
      ////qiLogInfo("Motion") << "MAX_ITER0: " << max_iter << std::endl;
      ...
      if (duration != 0) {
         ////qiLogInfo("Motion") << "MAX_ITERIF: " << max_iter << std::endl;
         inTime = duration / TEN_MS;
         ...
         ////qiLogInfo("Motion") << "MAX_ITERELSE: " << max_iter << std::endl;
...
void ActionGenerator::constructPose(std::string path) {
   ...
   ////qiLogInfo("Motion") << "ActionGenerator(" << file_name << ") creating" << endl;

   if (!in.is_open()) {
      qiLogFatal("Motion") << "ActionGenerator can not open " << file_name << endl;
   } else {
      int duration = 0;
      float stiffness = 1.0;
      float angles = 0.0;
   ...
   ////qiLogInfo("Motion") << "ActionGenerator(" << file_name << ") created" << endl;
}
