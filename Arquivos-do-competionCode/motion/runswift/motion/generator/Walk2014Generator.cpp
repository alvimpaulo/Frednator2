...
#define KICK_STEP_HEIGHT 0.045 // how far to lift kicking foot -----------------> Original 0.045
...
Walk2014Generator::Walk2014Generator(/*Blackboard *bb*/)
   ...
   initialise();
   ////qiLogInfo("Motion") << "Walk2014Generator constructed" << std::endl;

}

Walk2014Generator::~Walk2014Generator() {
   ////qiLogInfo("Motion") << "Walk2014Generator destroyed" << std::endl;
}

void Walk2014Generator::initialise() {
	////qiLogInfo("Motion") << "Walk2014 initializing" << endl;
...
/*void Walk2014Generator::avoidFeet(float &forward, float &left, float &turn, BodyModel& bodyModel) {
    ...
    vector<FootInfo> boxes	 = readFrom(vision, feetBoxes); //Extremamente problematico. Usa visão pra detectar BBox dos pes. Pensar no que fazer.
	...
	if (forward != 0 || left != 0 || turn != 0){
		...
		while (true){
			...
			//cout << forward << " " << left << " cutting original forward and left by 0.7" << endl;
				...
				left *= 0.7;
				clipped = true;
                if (bodyMoPrint the file name for each match.  This is the default when there is del.isLeftPhase){
					toeFall = Point(forward + toeOffset, left + footOffset);
				} else {
					toeFall = Point(forward + toeOffset, left - footOffset);
				}
...
//Yuri Notes: bodyModel -> used only for knowing current support feet and some ZMP value. Figuere out who set those parameters.
//            Odometry -> Not used, just get updated
//            SensorValues -> get joint values
//            ActionCommand -> get all the motion information from here
...
//		}
        ...
        //avoidFeet(f, l, turn, bodyModel); //AVOID FEET CALLED HERE
		forward = f /MM_PER_M;
		left = l / MM_PER_M;
...
void Walk2014Generator::makeForwardKickJoints(float kickLean, float kickStepH, float shiftPeriod, float &footh, float &forwardDist,
   ...
   // pe mais para frente. Original 0.7
   float kickPower;
   if (v4) {
   ...
   if (kickT < BACK_PHASE) {
      ...
      if (t >= pauseTime) {
          ...
          if (kickT2 >= halfShift*1.4) {
             float shiftedKickT = kickT2 - halfShift*1.4;
             float endT = BACK_PHASE - halfShift*1.4 - pauseTime;
...
void Walk2014Generator:: reset() {
   initialise();
   ////qiLogInfo("Motion") << "Walk2014 reset" << endl;
}

void Walk2014Generator:: stop() {
   // this does nothing
   stopping = true;
   ////qiLogInfo("Motion") << "Walk2014 stop" << endl;
}
