...
WalkEnginePreProcessor::WalkEnginePreProcessor(UnBoard *ub) {
   ...
   isKicking = false;
   //kickStruct.kickFinished = 0;
   writeTo(motion, kickFinished, 0);
   //kickFlagBoard.save(kickStruct);
...
JointValues WalkEnginePreProcessor::makeJoints(ActionCommand::All* request,
         ...
         if (isLineUpRequired(request->body.actionType)) {
            ...
            if (!lineUpEngine->hasEnded(request, ballX, ballY)) {
               ...
               if (request->body.actionType == Body::DRIBBLE) {
                  ...
                  dribbleEngine->preProcess(request, bodyModel); //USES BODY MODEL ISLEFTPHASE
                  active = Body::DRIBBLE;
   ...
   JointValues joints = walkEngine->makeJoints(request, odometry, sensors, bodyModel, ballX, ballY);    //WALK2014
...
//   cout << "kick state " << kickState << endl;
   ...
   if (walkEngine->active.actionType == Body::KICK && request->body.actionType == Body::WALK) {
      isKicking = false;
      int kFinished = readFrom(motion, kickFinished);
      if( kFinished == 2)
      {
         ////qiLogInfo("Motion") << "isKicking Stop" << std::endl;
         //kickStruct.kickFinished = 1;
         writeTo(motion, kickFinished, 1);
         //kickFlagBoard.save(kickStruct);
      }

   } else {
      ////qiLogInfo("Motion") << "isKicking Active" << std::endl;
      //kickStruct.kickFinished = 2;
      writeTo(motion, kickFinished, 2);
      //kickFlagBoard.save(kickStruct);
